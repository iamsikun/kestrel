...
| [`reporting.py`](../src/kestrel/reporting.py) | The one verified interpretation of a campaign outcome, shared by `Lab.report` and every messaging surface | Bookkeeping over recorded evidence; it does not establish that a claim is correct |
| [`sources.py`](../src/kestrel/sources.py) | Bounded read-only readers over the controller and evidence stores, with schema validation, cursors and source-identity binding | An application boundary, not an OS-enforced one; actual write denial is a deployment property |
| [`briefings.py`](../src/kestrel/briefings.py) | Deterministic read models and templates; named derivations, charged reservations, explicit unavailable fields | No language model; a briefing cannot state anything the verified report would refuse |
| [`messaging.py`](../src/kestrel/messaging.py) | Assistant database, projection and rules, subscriptions, versioned milestones, DST-correct schedules, attention state, inbox, inbound command processing | Attention is not resolution; resolution is always read back from source state |
| [`notifications.py`](../src/kestrel/notifications.py) | Operator-issued channels and grants, disclosure and rendering, immutable envelopes, release permits, the delivery state machine, quotas, offline transports | `ACCEPTED` means a transport accepted a request, never delivery, reading, review or approval |
| [`telegram.py`](../src/kestrel/telegram.py) | Four-method typed HTTPS adapter, credential handling and redaction, pairing, credential rotation, durable inbound poller | Behind an activation gate; no live integration has been performed and none is claimed |
